...
- `src/cli.ts` 的 `hook-enforce` / `permission-enforce` 分支、`src/hook-runner.ts`、`src/post-tool-audit.ts`、`src/session-lifecycle.ts`：hook 拦截、授权、审计和生命周期输出
