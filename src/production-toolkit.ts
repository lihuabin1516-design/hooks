...
function createInstallScript(input: CreateProductionToolkitInput): string {
  ...
  return [
    ...
    'foreach ($name in @("SessionStart","PreToolUse","PermissionRequest","PostToolUse","Stop")) {',
...
function createBootstrapScript(input: CreateProductionToolkitInput): string {
  return [
    ...
    ')',
    '$ErrorActionPreference = "Stop"',
    `node ${psSingleQuote(path.join(input.prototypeRoot, 'dist', 'src', 'cli.js'))} write-current --root $ProjectRoot --task-id $TaskId --phase $Phase`,
    'Write-Output "Bootstrap complete. Start Codex in the project root to activate lifecycle hooks."',
    ''
  ].join('\n');
}
...
function createReadme(input: CreateProductionToolkitInput): string {
  return [
    ...
    '- `BOOTSTRAP-PROJECT.ps1`: writes `<project>/.ccpanes-task/current-task.json`.',
